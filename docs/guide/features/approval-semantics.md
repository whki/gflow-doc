...
const matrixGroups = [
  ...
  {
    title: '办理中动作',
    items: ['加签', '减签', '转办', '委托', '签收/抢单', '退回上一节点', '撤回', '评论', '附件'],
  },
  {
    title: '候选人类型',
...
审批中途动态插入审批人（加签）或移除未办理的会签人（减签）。加签为**前加签**语义：新加签人先审，加签子任务全部完成后原审批人再出结果。引擎通过 `wf_task.parent_id + sequence_order` 维护任务父子链，加签产生子任务，主任务等待链上任务全部完成。
...
- **转办**：任务转给他人办理，办理人变更，留痕写入任务变量 `transfer_from` / `transfer_reason` / `transfer_time`
- **委托**：受托人代办，`owner` 保留原拥有人、`delegate_from` 落表留痕；被委派人处理后经 `resolve` 归还原审批人
