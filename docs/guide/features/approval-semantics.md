...
| `autoApprove` | 保留发起人（预留自动通过语义，当前实现不过滤） |
...
| `toNode` | 跳到 `target` 指定节点（必填，须为链中存在的节点 ID） |
...
部署 / 更新流程时，引擎对各节点 `configuration` 做校验：未知取值、必填缺失、互斥组合（如票签阈值配在非票签节点、`toNode` 缺 `target`）会**直接拒绝部署**，并给出节点级错误信息——配置错误拦在写入前而不是运行期。
